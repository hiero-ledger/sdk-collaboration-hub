...
function algorithmFromKeyObject(keyObject: KeyObject): KeyAlgorithm {
  ...
  throw new Error(`Unsupported private key type: ${keyObject.asymmetricKeyType ?? "unknown"}`);
}
...
export class PrivateKey extends Key {
  ...
  public createPublicKey(): PublicKey {
    return PublicKey.fromPEM(createPublicKey(this.keyObject).export({ format: "pem", type: "spki" }).toString());
  }
