...
1. **Conflicts with community direction on freeze deprecation** — [HIP-745](https://github.com/hiero-ledger/hiero-improvement-proposals/blob/main/HIP/hip-745.md) explicitly calls for deprecating `freeze()` as a required step, noting that it "doesn't appear to add much value" and creates "unnecessary additional coding steps and complexity." It also requires that transactions be serializable *without* freezing — enabling dApps to send incomplete transactions (missing `nodeAccountId`, `transactionId`) to wallets for completion. `PackedTransaction` would have moved in the opposite direction, making freeze **mandatory** at the type level — `pack()` was semantically identical to `freezeWith()`, just returning a new type. See also [Issue #56](https://github.com/hiero-ledger/sdk-collaboration-hub/issues/56) and its linked discussions ([hiero-sdk-js#1445](https://github.com/hiero-ledger/hiero-sdk-js/issues/1445), [hedera-sdk-reference#127](https://github.com/hashgraph/hedera-sdk-reference/issues/127)).
