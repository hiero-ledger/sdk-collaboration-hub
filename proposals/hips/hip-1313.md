...
TransactionRecord {
    ...
    // Only present when the transaction used high-volume throttles.
    @@nullable @@immutable
    uint64 highVolumePricingMultiplier
}
```

## Supported Transactions

The following transaction types support the high-volume throttle option:

- TopicCreate
- ContractCreate
- ApproveAllowance
- AccountCreate
- Transfer (for hollow account creation)
- FileCreate
- FileAppend
- HookStore
- ScheduleCreate
...
1. **Given** an AccountCreateTransaction is configured with `setHighVolume(true)`, **when** the transaction is executed, **then** the account is created successfully using high-volume throttles and the `highVolumePricingMultiplier` field in the TransactionRecord is populated with a value greater than or equal to 1000.
