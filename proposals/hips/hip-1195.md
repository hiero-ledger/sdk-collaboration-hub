...
type EvmHookSpec {
    // The source of the EVM bytecode for the hook - non-nullable
...
abstract type LambdaStorageUpdate {}

type LambdaStorageSlot extends LambdaStorageUpdate {
    // 32-byte storage slot key - non-nullable
    byte[] key

    // 32-byte storage slot value (empty to delete) - non-nullable
    byte[] value
}

type LambdaMappingEntries extends LambdaStorageUpdate {
    // Slot corresponding to Solidity mapping
    byte[] mappingSlot - non-nullable

    // The entries in the mapping at the given slot - non-nullable
    List<LambdaMappingEntry> entries

}

type LambdaMappingEntry {
    // oneof key or preimage
    // 32-byte key of mapping entry - nullable
    byte[] key

    // The bytes that are the preimage of the Keccak256 hash that forms the mapping key - nullable
    byte[] preimage

    // 32-byte value of mapping entry (empty to delete) - non-nullable
    byte[] value
}
```
...
absract type HookCall {
    // The id of the hook to call.
    int64 hookId

    // Specification of how to call an EVM hook - non-nullable
    EvmHookCall evmHookCall 
}

type FungibleHookCall extends HookCall {
    FungibleHookType hookType
}

type NftHookCall extends HookCall {
    NftHookType hookType
}
...
type EvmHookCall {
    // Call data to pass to the hook - non-nullable
    byte[] data
...
**Methods:**
- Getters and setters
- `addStorageUpdate(LambdaStorageUpdate)`
- `setStorageUpdates(List<LambdaStorageUpdate>)`
...
- `addHookToCreate(HookCreationDetails)` - Add a hook to be created for the account
- `setHooksToCreate(List<HookCreationDetails>)` - Set hooks to be created with the account
- `addHookToDelete(int64)` - Mark a hook for deletion from the account
- `setHooksToDelete(List<int64>)` - Mark hooks for deletion from the account
...
- `addHbarTransferWithHook(accountId, amount, fungibleHookCall)` - Add HBAR transfer with hook (PRE_HOOK or PRE_POST_HOOK)

**NFT Transfer Methods with Hooks:**

- `addNftTransferWithHook(nftId, senderAccountId, receiverAccountId, senderNftHookCall, receiverNftHookCall)` - Add NFT transfer with receiver hook (PRE_HOOK_SENDER, PRE_POST_HOOK_SENDER, PRE_HOOK_RECEIVER or PRE_POST_HOOK_RECEIVER)

**Token Transfer Methods with Hooks:**

- `addTokenTransferWithHook(tokenId, accountId, amount, fungibleHookCall)` - Add token transfer with hook (PRE_HOOK or PRE_POST_HOOK)
...
31. **when** a LambdaSStoreTransaction attempts to update storage with non-existent HookId, **then** the transaction fails with `HOOK_NOT_FOUND`.
