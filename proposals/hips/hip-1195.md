...
- `addHookToDelete(uint64)` - Mark a hook for deletion from the account
- `setHooksToDelete(List<uint64>)` - Mark hooks for deletion from the account
...
- `addNftTransferWithHook(nftId, senderAccountId, receiverAccountId, hookCall, senderHookType, receiverHookType)` - Add NFT transfer with receiver hook (PRE_HOOK_SENDER, PRE_POST_HOOK_SENDER, PRE_HOOK_RECEIVER or PRE_POST_HOOK_RECEIVER)
