...
#### LambdaEvmHook
```
// Definition of a lambda EVM hook extending shared specifications for an EVM hook
LambdaEvmHook {
    // The source of the EVM bytecode for the hook
    contractId: ContractId

    // Initial storage updates for the lambda, if any
    @optional storageUpdates: list<LambdaStorageUpdate>
}
```
...
LambdaMappingEntries {
    ...
    entries: list<LambdaMappingEntry>
}
...
HookStoreTransaction {
    ...
    storageUpdates: list<LambdaStorageUpdate>
    
    // Add a single storage update to the transaction
    HookStoreTransaction addStorageUpdate(update: LambdaStorageUpdate)
    
    // Set all storage updates for the transaction
    HookStoreTransaction setStorageUpdates(updates: list<LambdaStorageUpdate>)
    
    // Get the list of storage updates
    list<LambdaStorageUpdate> getStorageUpdates()
...
29. **Given** a lambda hook exists with storage, **when** a HookStoreTransaction updates storage slots with valid signatures, **then** the storage is updated successfully.

30. **Given** a lambda hook exists, **when** a HookStoreTransaction attempts to update storage without proper signatures, **then** the transaction fails with an unauthorized error.
