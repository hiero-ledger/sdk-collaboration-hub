...
9. **Given** an `EthereumTransactionDataEip7702` object is constructed with authorization data, **when** it is passed to `setEthereumTransaction()` on an EthereumTransaction, **then** the transaction executes successfully and the code delegation is set according to the authorization entries.

10. **Given** an `EthereumTransactionDataEip7702` object is constructed with authorization data, **when** `toBytes()` is called on it and the resulting bytes are set using `setEthereumTransaction()` on an EthereumTransaction, **then** the transaction executes successfully and the code delegation is set according to the authorization entries.
