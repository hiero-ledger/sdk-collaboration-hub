...
Describes a block node endpoint, including which block node API it supports.

```
@@finalType
BlockNodeEndpoint {
    @@immutable endpointApi: BlockNodeApi
}
```

### MirrorNodeEndpoint

Describes a mirror node endpoint. Currently carries no additional data.
...
Describes an RPC relay endpoint. Currently carries no additional data.
...
> **Design rationale -- marker types vs. enum:** `MirrorNodeEndpoint` and `RpcRelayEndpoint` are intentionally defined as distinct (currently empty) types rather than collapsing endpoint kind into a single enum. The protobuf specification defines these as separate messages so that type-specific fields can be added in future HIPs without restructuring the message. Preserving this pattern in the SDK means each endpoint type can independently gain fields (e.g., mirror node API version, RPC relay chain ID) without introducing breaking API changes. An enum-based approach would require bolting on new nullable companion types later, effectively retrofitting the oneOf pattern after the fact.
...
@@oneOf(blockNode, mirrorNode, rpcRelay)
@@finalType
RegisteredServiceEndpoint {
    @@immutable @@nullable ipAddress: bytes
    @@immutable @@nullable domainName: string
    @@immutable port: uint32
    @@immutable @@default(false) requiresTls: bool
    @@immutable @@nullable blockNode: BlockNodeEndpoint
    @@immutable @@nullable mirrorNode: MirrorNodeEndpoint
    @@immutable @@nullable rpcRelay: RpcRelayEndpoint
}
```
...
RegisteredNodeCreateTransaction {
    @@getSet adminKey: Key
    @@getSet @@nullable description: string
    @@getSet serviceEndpoints: list<RegisteredServiceEndpoint>
...
```
RegisteredNodeDeleteTransaction {
    @@getSet registeredNodeId: uint64
}
```
...
```
NodeCreateTransaction {
    @@getSet associatedRegisteredNodes: list<uint64>
...
HIP-1137 does not define any new consensus node response codes. As a result, the Response Codes and Transaction Retry sections are omitted from this document.

If response codes specific to registered node transactions are introduced during consensus node implementation, this design document should be updated to document them and evaluate any transaction retry implications for the SDKs.
...
2. Create a `RegisteredServiceEndpoint` with an IP address, port, TLS enabled, and a `BlockNodeEndpoint` configured with `BlockNodeApi.SUBSCRIBE_STREAM`.
...
7. Create a second `RegisteredServiceEndpoint` with a domain name, port, TLS enabled, and a `BlockNodeEndpoint` configured with `BlockNodeApi.STATUS`.
