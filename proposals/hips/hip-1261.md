...
- Behavior:
  ...
  - The response SHOULD include a `notes` entry indicating the number of chunks used (e.g., `"chunks: 3"`). SDKs MAY optionally expose a per‑chunk breakdown in debug/verbose output, but the primary API returns a single aggregated estimate.
