...
Abstract base type for streaming queries. **StreamQuery** defines stream-specific methods and shared configuration for retry and deadline behavior. **BlockStreamQuery** extends **StreamQuery&lt;Block&gt;**. **TopicMessageQuery** should also extend **StreamQuery&lt;TopicMessage&gt;** if doing so is not a breaking change in the SDK's language; if it would be a breaking change, the SDK should implement the same handler and subscribe interface on **TopicMessageQuery** without requiring it to extend **StreamQuery**. Handlers need not be set by the caller; if a handler is not set, the SDK uses a default (e.g. no-op or logging).

```
abstraction StreamQuery<$$Item> {
    // Set the gRPC deadline for stream operations
    StreamQuery setGrpcDeadline(deadline: duration)

    // Set the maximum number of retry attempts
    StreamQuery setMaxAttempts(maxAttempts: int32)

    // Set the maximum backoff duration between retries
    StreamQuery setMaxBackoff(maxBackoff: duration)
...
Concrete stream queries add only their own configuration: **BlockStreamQuery** adds **startBlockNumber** and **endBlockNumber**; **TopicMessageQuery** adds **setTopicId**, **setStartTime**, **setEndTime**, **setLimit**, and their getters.
...
@@finalType
ServerStatusQuery {
    // Set the gRPC deadline for this query
    ServerStatusQuery setGrpcDeadline(deadline: duration)

    // Set the maximum number of retry attempts
    ServerStatusQuery setMaxAttempts(maxAttempts: int32)

    // Set the maximum backoff duration between retries
    ServerStatusQuery setMaxBackoff(maxBackoff: duration)
...
BlockQuery {
    ...
    @@nullable blockNumber: uint64

    // When true, request the latest available block. Mutually exclusive with blockNumber.
    @@nullable retrieveLatest: bool

    // Set the gRPC deadline for this query
    BlockQuery setGrpcDeadline(deadline: duration)

    // Set the maximum number of retry attempts
    BlockQuery setMaxAttempts(maxAttempts: int32)

    // Set the maximum backoff duration between retries
    BlockQuery setMaxBackoff(maxBackoff: duration)
...
**Convenience methods (`forTestnet()`, `forMainnet()`, `forPreviewnet()`):** These methods do not currently set default Block Node addresses because the Block Node infrastructure is new and public network addresses are not yet established. Once Block Node addresses are known for the public networks — either hardcoded or discovered dynamically via the on-chain registry defined in [HIP-1137](https://github.com/hiero-ledger/hiero-improvement-proposals/blob/main/HIP/hip-1137.md) — the convenience methods should be updated to include default Block Node network configuration. Until then, users must call `setBlockNodeNetwork()` explicitly.
...
Block Node queries (**ServerStatusQuery**, **BlockQuery**, **BlockStreamQuery**) do **not** extend the SDK's existing `Executable` base type. `Executable` carries consensus-network concerns (`setNodeAccountIds`, query payment logic) that do not apply to Block Node queries — Block Nodes are addressed by `host:port`, not by account ID, and do not require query payment. Instead, Block Node queries define their own **setGrpcDeadline**, **setMaxAttempts**, **setMaxBackoff**, and **execute** (or **subscribe** for streaming queries) methods directly. SDKs should implement connection management, retry handling, and error mapping following their language's idiomatic patterns.

Note: **BlockStreamQuery** extends **StreamQuery&lt;Block&gt;**, which provides the shared streaming configuration and handler methods. Common subscription and handler behavior is implemented in **StreamQuery** only.
...
4. Request/response queries (**ServerStatusQuery**, **BlockQuery**) define their own `execute(client)` method and shared configuration (**setGrpcDeadline**, **setMaxAttempts**, **setMaxBackoff**). Streaming queries (**BlockStreamQuery**) extend **StreamQuery**, which provides the same shared configuration along with **subscribe(client, onNext)** (invokes the callback for each item, returns a **SubscriptionHandle** with **unsubscribe()**), **setCompletionHandler**, **setErrorHandler**, and **setRetryHandler**. There is no **execute** method on stream queries; callers subscribe and collect items in the callback until the stream ends or they unsubscribe.
