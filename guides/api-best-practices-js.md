...
JavaScript has no native interface or abstract class syntax. The meta-language `contract` and `abstraction` keywords map to either
duck-typing conventions or `class` usage depending on the keyword used.

### Contract types (`contract` keyword)

JavaScript has no interfaces. Document the contract using JSDoc `@typedef` or `@callback` annotations and rely on
duck-typing. Consumers satisfy the contract by implementing the documented methods.

```javascript
/**
 * @typedef {Object} Executable
 * @template R
 * @property {function(HieroClient): Promise<R>} execute
 */

/**
 * @typedef {Object} Subscribable
 * @template T
 * @property {function(function(): void): Subscribable} setCompletionHandler
 * @property {function(HieroClient, function(T): void): Promise<SubscriptionHandle>} subscribe
 */
```

### Base types (`abstraction` keyword)

Implement as a `class` with shared fields and methods. Subtypes extend the class. Methods that subclasses must
override should throw an error in the base to signal they are abstract.
...
class Request {
    ...
    /** @protected */
    withRetry(network, action, shouldRetry) {
...
Extend the base class. Since JavaScript has no interfaces, contract satisfaction is implicit — the subclass simply
implements the methods that the contract defines.

```javascript
// Meta-language:
//   abstraction Transaction extends ConsensusRequest, Executable<TransactionResponse> { ... }

class Transaction extends ConsensusRequest {
    // Satisfies Executable by implementing execute():
    async execute(client) {
        return this.withRetry(this.getNetwork(client), (node) => { /* ... */ }, (err) => this.shouldRetry(err));
    }
}
```
...
JavaScript has no `protected` keyword. Use `#` private fields for truly internal state, or prefix methods with `_`
and document them as `@protected` via JSDoc to signal they are for subclass use only.

### Callback parameters

Map callbacks to plain JavaScript functions:

| Meta-language | JavaScript |
|---|---|
| `void callback()` | `function()` / `() => {}` |
| `void callback(item: $$T)` | `function(item)` |
| `bool callback(error: $$Error)` | `function(error): boolean` |
| `$$R callback(node: $$N)` | `async function(node)` |
