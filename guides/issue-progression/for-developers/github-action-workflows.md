...
- [How to Pin GitHub Actions to a Specific Commit Hash](#how-to-pin-github-actions-to-a-specific-commit-hash)
...
When creating or updating GitHub workflows in the Hiero SDKs, all GitHub Actions must be pinned to a specific commit hash rather than using floating tags.
...
uses: owner/action@<commit>    # ✅ pinned commit SHA
```
...
- Keep a version comment (for example `# v2.14.0`) for easy reference
